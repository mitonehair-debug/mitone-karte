...
const VISIT_SOURCES = [“ご紹介”,“SNS（Instagram・note等）”,“ホットペッパービューティー”,“ネット検索（Google等）”,“通りがかり・近所”,“チラシ・看板”,“その他”];
...
const YEARS = Array.from({length:100},(*,i)=>String(new Date().getFullYear()-i));
const MONTHS = Array.from({length:12},(*,i)=>String(i+1));
...
try{
const ref=await addDoc(collection(db,“customers”),{…cForm,visits:[],purchases:[],createdAt:serverTimestamp()});
setCustomers([{…cForm,id:ref.id,visits:[],purchases:[]},…customers]);
...
try{
await updateDoc(doc(db,“customers”,selId),cForm);
setCustomers(customers.map(c=>c.id===selId?{…c,…cForm}:c));
setEditingCustomer(false);showToast(“更新しました”);
...
if(c._docId)await deleteDoc(doc(db,“pending”,c.*docId));
setCustomers([{…data,id:ref.id},…customers]);
setPending(pending.filter((*,i)=>i!==idx));showToast(“カルテに登録しました”);
}catch(e){showToast(“登録に失敗しました”,“error”);}
}
...
setTrash([{…customer,originalId:id},…trash]);
setView(“list”);showToast(“ゴミ箱に移しました”);
...
setCustomers([{…data,id:ref.id},…customers]);
setTrash(trash.filter(x=>x.id!==tid));showToast(“復元しました”);
...
setTrash(trash.filter(x=>x.id!==t.id));showToast(“完全に削除しました”);
}catch(e){showToast(“削除に失敗しました”,“error”);}
}
...
// 来店記録内の購入記録を商品購入履歴に自動反映
const allPurchases=newVisits.flatMap(v=>
(v.purchases||[]).map(p=>({…p,visitDate:v.date,visitId:v.id}))
);
try{
await updateDoc(doc(db,“customers”,selId),{visits:newVisits,purchases:allPurchases});
setCustomers(customers.map(c=>c.id!==selId?c:{…c,visits:newVisits,purchases:allPurchases}));
setVForm(eVisit);setEditVid(null);setVTouched(false);setView(“detail”);
  ...
  <main style={{maxWidth:640,margin:"0 auto",padding:"16px 14px 60px"}}>
    ...
    {view==="list"&&(
      <div>
        ...
        {loading?<div style={{textAlign:"center",padding:"48px",color:SUB}}>⏳ 読み込み中...</div>:
          ...
          filtered.map(c=>{
            const l=lastVisit(c);const d=l?daysSince(l):null;const isLost=d&&d>=alertDays;
            ...
            return(
              <div key={c.id} onClick={()=>{setSelId(c.id);setEditingCustomer(false);setView("detail");}} style={{background:"#fff",borderRadius:12,padding:"12px 14px",display:"flex",alignItems:"center",gap:12,cursor:"pointer",border:`1.5px solid ${isLost?"#f5c6a0":BORDER}`,marginBottom:8}}>
                ...
                {visitCount>0&&(
                  <div style={{textAlign:"center"}}>
                    <span style={{display:"block",fontSize:19,fontWeight:800,color:GOLD}}>{visitCount}</span>
                    <span style={{fontSize:10,color:SUB}}>来店</span>
                  </div>
                )}
              </div>
            );
      ...
      <div style={fw}>
        ...
        <div style={card}>
          ...
          <div style={{marginBottom:13}}>
            <label style={lbl}>来店のきっかけ{opt}</label>
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
    ...
    {view==="detail"&&sel&&(
      <div style={fw}>
        ...
        {/* 商品購入履歴（来店記録の購入記録が自動反映） */}
        <div style={{margin:"18px 0 8px"}}>
          <h3 style={{fontSize:14,fontWeight:800,margin:0}}>🛍️ 商品購入履歴</h3>
        </div>
        {!(sel.purchases&&sel.purchases.length)?
          <div style={{color:SUB,fontSize:13,padding:"12px 0",textAlign:"center"}}>購入履歴がありません</div>:
          (sel.purchases||[]).map((p,i)=>(
            <div key={p.id||i} style={{...card,padding:"10px 14px",marginBottom:7}}>
              <div style={{fontSize:12,color:GOLD,fontWeight:700,marginBottom:4}}>📅 {p.visitDate?p.visitDate.replace("T"," "):p.date}</div>
              <div style={{fontSize:14,fontWeight:600}}>{p.item}</div>
              {p.price&&<div style={{fontSize:13,color:SUB,marginTop:2}}>¥{p.price}</div>}
              {p.memo&&<div style={{fontSize:12,color:SUB,marginTop:2}}>{p.memo}</div>}
            </div>
          ))
        }
      </div>
