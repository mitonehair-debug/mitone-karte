...
R: { name: "技術職人型", color: "#C17B3F", lines: ["技術を磨くことそのものに喜びを感じるあなたは、施術のクオリティにとことんこだわる美容師です。","手を動かし、思い通りのヘアスタイルを仕上げた瞬間の達成感が、仕事で一番の原動力になっています。","技術の専門家として、技術のクオリティや施術、理論を極めるスペシャリストの道があなたに向いています。"],
  weakness: "施術の精度や手技の安定感を意識的に高める機会が少ないかもしれません。「なんとなくできる」に慣れず、一つひとつの手技に意図を持つことで、技術への信頼がさらに増します。" },
I: { name: "研究探求型", color: "#4A7FA5", lines: ["「なぜそうなるのか」を知りたい気持ちが強いあなたは、感覚だけでなく理論で美容を理解しようとします。","髪質・薬剤・トレンドの背景まで深く調べることで、お客様への提案に説得力が生まれます。","知識を発信する講師やセミナー活動など、学びを広げるキャリアにも向いています。"],
  weakness: "理論や知識を深く調べることへの関心が低めかもしれません。「感覚でやっている」が続くと、お客様への説明や提案の説得力が弱くなることがあります。薬剤や技術の仕組みを少し掘り下げると、信頼感が大きく変わります。" },
A: { name: "クリエイター型", color: "#8A5BA8", lines: ["美的センスと独自の発想を持つあなたは、ヘアデザインを通じて自己表現することに喜びを感じます。","流行をただ追うのではなく、自分のスタイルをお客様に提案できるのが最大の強みです。","コレクションやSNS発信など、クリエイティブな方向でブランドを築くことがあなたに合っています。"],
  weakness: "デザインや「自分らしさ」を表現することへの意識が低めかもしれません。お客様が持つ「なりたいイメージ」の幅に対応するために、ビジュアルのインプットを意識的に増やすことが助けになります。" },
S: { name: "コミュニケーター型", color: "#4A9068", lines: ["人と話し、寄り添うことが自然にできるあなたは、お客様との関係そのものを大切にする美容師です。","悩みを引き出し、安心感を与える力が、リピーターやご紹介につながる大きな武器になっています。","カウンセリングや後輩育成など、人との関わりを深めるキャリアでさらに輝けます。"],
  weakness: "人の気持ちに寄り添ったり、言葉の裏にある想いを汲み取ることが苦手な場面があるかもしれません。お客様が言葉にしていない要望を引き出す力が、リピートとご紹介を生む大きな鍵になります。" },
E: { name: "経営リーダー型", color: "#C45A3A", lines: ["全体を見渡し、引っ張っていくことに意欲を感じるあなたは、サロンの未来を考えながら動ける美容師です。","新しいメニューや仕組みを考えることが苦にならず、チームのモチベーションを上げる力があります。","独立・開業・マネジメントなど、経営に関わるキャリアが自然なステップになるでしょう。"],
  weakness: "集客・価格設定・自己PRなど、ビジネス的な動きへの苦手意識があるかもしれません。独立や昇格を目指すとき、「技術はある、でも経営がわからない」という壁がここから来ることが多いです。" },
C: { name: "管理サポート型", color: "#5A7A8A", lines: ["正確さと丁寧さを大切にするあなたは、サロンの土台を支える存在です。","カルテ管理・在庫・予約など、細かい部分まで気を配れる力が、チーム全体の安心感につながっています。","サロンの縁の下の力持ちとして、チームや仕組みを整える役割であなたの力が最大限に活きます。"],
  weakness: "記録・管理・手順の徹底が課題になりがちです。カルテ管理の精度やスケジュール管理を丁寧にするだけで、お客様からの信頼が着実に高まります。「後でまとめてやる」を減らすことから始めてみましょう。" },
};
...
【このアプリの位置づけ】
これは七海と話すための「入り口」です。目的は答えを出すことではなく、相手が「もっと七海と話したい」と思う状態を作ることです。3回の会話を通じて、相手の中に「問い」を残してください。
...
・「〜ています」を使う（「〜てる」は使わない）
・「でも」→「しかし」、「やっぱり」→「やはり」
・箇条書きの羅列・AIっぽい硬い言い回しは使わない
・二重引用符（""）は使わない

【3回の会話の役割】

■ 1回目：鏡を見せる（ハッとさせる）
診断結果を「鏡」として見せてください。ただし毎回必ず異なる切り口から見ること。
以下の視点をランダムに選んで使ってください（毎回同じ視点を使わない）：
- 自己評価と他者評価のギャップに注目する
- 一番高いスコアより「一番低いスコア」に注目する
- スコアではなく「組み合わせのパターン」に注目する
- 「他者評価が自己評価より高い項目」に注目する
- 「自分が思っていないのに周りがそう見ている強み」に注目する
最後は「一つだけ聞いてもいいですか？」か「〜はどうでしょうか？」で相手に返す。

■ 2回目：やさしく揺さぶる（デビルズアドボケート）
相手の返答を受けて、一つだけ「でも、本当にそうでしょうか？」という視点を入れてください。
相手が思い込んでいるかもしれないことに、優しく疑問を投げかける。
例：「技術が低いとおっしゃいますが、それは本当に技術の問題でしょうか？それとも、伝え方の問題かもしれません」
例：「忙しいからできないとおっしゃいますが、実は忙しさを理由にしている部分はないでしょうか？」
決めつけず、「〜という可能性はないでしょうか？」という形で問いかける。
最後は「どう思いますか？」で締める。

■ 3回目：問いを残して終わる（続きは直接）
具体的なアクションを一つだけ提案してください。
しかし「答え」は出し切らない。意図的に一つ「問い」を残して終わること。
最後は必ず以下の形で締める：
「今日お話しした内容、もう少し掘り下げてみる価値があると思います。もしよければ、直接話しませんか？」
「応援しています」で締めても良いが、その後に上記の一文を必ず添える。

【哲学・信念】
美容師の離職率の高さや将来への不安を、個人の根性論ではなく「仕組み」で解決することが役割。スタッフが10年後・20年後に自分の足で立って笑っていられるよう、生存戦略を一緒に練ること。一番大切にしているのは「誠実さ」と「共感」。表面的な技術だけでなく、一人の人間としての「あり方」を何よりも重視している。

【タイプ別のデビルズアドボケート視点】
技術職人タイプ(R)：「技術を磨くことに時間をかけていますが、その価値はお客様に伝わっていますか？」
研究探求タイプ(I)：「知識は豊富だと思いますが、それをお客様の言葉で説明できていますか？」
クリエイタータイプ(A)：「センスがあると思いますが、自分の世界観を相手に押しつけていることはないですか？」
コミュニケータータイプ(S)：「人に合わせることが得意なのはわかりました。でも、あなた自身の意見はどこにありますか？」
経営リーダータイプ(E)：「全体を見ることが得意なようですが、目の前の一人に向き合う時間は取れていますか？」
管理サポートタイプ(C)：「丁寧さは武器ですが、それが自分の意見を言えない理由になっていることはないですか？」

【よく使う表現】
「自分を確立させる」「掘り下げる」「仕組み化」「持ち味」「本質的には」「やはり」

【注意】
・「チームや後輩」など特定の立場を前提にした表現は避け、「周囲の人」「関わる人」など誰にでも当てはまる言葉を使う
・相手の職場環境・立場は分からないため、決めつけずに問いかける形にする
・200〜300字程度の自然な語りかけで
・同じデータでも毎回必ず違う切り口・違う表現でコメントすること
・1回目のコメントでは、強みだけでなく「スコアが低かった軸」にも自然に触れ、成長の余白として前向きに伝えること。ただし責めるような言い方は絶対にしない`;
...
<div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"20px 16px 40px", maxWidth:"480px", margin:"0 auto", width:"100%", boxSizing:"border-box" }}>
...
useEffect(() => {
  fetch("/api/stats").then(r => r.json()).then(d => setDiagCount(d.count)).catch(() => {});
}, []);
...
<p style={{ fontSize:"13px", color:"#8A7E74", marginBottom:"16px", lineHeight:"1.8" }}>30問で、自分でも知らなかった強みが見つかる。<br/>気になったら、七海に直接相談できます。</p>
{diagCount !== null && diagCount > 0 && (
  <div style={{ background:"#F0F7F3", borderRadius:"50px", padding:"6px 18px", marginBottom:"24px", display:"inline-flex", alignItems:"center", gap:"6px" }}>
    <span style={{ fontSize:"16px" }}>✅</span>
    <span style={{ fontSize:"12px", color:"#4A9068", fontWeight:"700" }}>これまで {diagCount} 人が診断済み</span>
  </div>
)}
  ...
  {/* noteへの導線 */}
  <div style={{ marginTop:"28px", maxWidth:"360px", width:"100%", borderTop:"1px solid #EDE7DF", paddingTop:"20px" }}>
    <p style={{ fontSize:"11px", color:"#A09488", marginBottom:"10px", letterSpacing:"0.1em" }}>七海の発信を読む</p>
    <a href="https://note.com/nanaumi_mitone" target="_blank" rel="noopener noreferrer"
      style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:"#fff", borderRadius:"12px", padding:"14px 18px", textDecoration:"none", boxShadow:"0 2px 12px rgba(0,0,0,0.05)", border:"1px solid #EDE7DF" }}>
      <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
        <span style={{ fontSize:"20px" }}>📝</span>
        <div style={{ textAlign:"left" }}>
          <p style={{ fontSize:"13px", fontWeight:"700", color:"#2C2416", margin:0 }}>美容師のキャリアを考えるnote</p>
          <p style={{ fontSize:"11px", color:"#8A7E74", margin:0, marginTop:"2px" }}>七海隼｜MITONE｜群馬県伊勢崎市</p>
        </div>
      </div>
      <span style={{ fontSize:"14px", color:"#A09488" }}>→</span>
    </a>
  </div>
</div>
  ...
  try {
    ...
    if (navigator.share) {
      if (navigator.canShare && navigator.canShare({ files:[file] })) {
        // ファイル共有対応（Android Chrome等）
        await navigator.share({ files:[file], title:"私のキャリアタイプは「"+info.name+"」でした", text:"美容師向け無料キャリア診断やってみた！\n→ mitone-karte-ahdo.vercel.app" });
      } else {
        // iOS Safari等：URL共有にフォールバック
        await navigator.share({ title:"美容師タイプ別キャリア診断｜MITONE", text:"私のキャリアタイプは「"+info.name+"」でした！\n美容師向け無料診断 →", url:"https://mitone-karte-ahdo.vercel.app" });
      }
    } else {
      // デスクトップ等：画像ダウンロード
...
<h1 style={{ color:"#fff", fontSize:"28px", fontWeight:"700", margin:0 }}>{info.name}</h1>
  ...
  <div style={{ maxWidth:"480px", margin:"0 auto", padding:"0 16px", boxSizing:"border-box", width:"100%" }}>
    ...
    {/* 成長のヒント */}
    {(() => {
      const sorted = [...KEYS].sort((a,b) => selfScores[a] - selfScores[b]);
      const bottom2 = sorted.slice(0,2);
      return (
        <div style={{ background:"#fff", borderRadius:"16px", padding:"24px", marginTop:"16px", boxShadow:"0 2px 16px rgba(0,0,0,0.06)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"20px" }}>
            <span style={{ fontSize:"18px" }}>🌱</span>
            <p style={{ fontSize:"11px", color:"#A09488", letterSpacing:"0.15em", fontWeight:"600", margin:0 }}>成長のヒント</p>
          </div>
          <p style={{ fontSize:"12px", color:"#8A7E74", marginBottom:"16px", lineHeight:"1.7" }}>あなたのスコアが低めだった2つの軸から、意識すると伸びる視点をお伝えします。</p>
          {bottom2.map((key, i) => (
            <div key={key} style={{ marginBottom: i===0 ? "16px" : 0, paddingBottom: i===0 ? "16px" : 0, borderBottom: i===0 ? "1px solid #F0EAE2" : "none" }}>
              <div style={{ display:"flex", alignItems:"center", gap:"6px", marginBottom:"8px" }}>
                <span style={{ fontSize:"11px", fontWeight:"700", color:"#8A5030", background:"rgba(193,123,63,0.1)", padding:"3px 10px", borderRadius:"4px" }}>{AXIS_LABELS[key]}</span>
              </div>
              <p style={{ fontSize:"13px", lineHeight:"1.85", color:"#4A4036", margin:0 }}>{TYPE_INFO[key].weakness}</p>
            </div>
          ))}
        </div>
      );
