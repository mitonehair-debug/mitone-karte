...
const AI_FOLLOW_UP_MESSAGES = [
  "",
  "もう少し詳しく教えてほしいな。特に自分では気づきにくい部分や、今の強みをどう活かせるか、別の角度から見てもらえますか？",
  "最後に、今すぐできる具体的なアクションをひとつだけ教えてもらえますか？",
];
...
const newMessages = aiRound === 0
  ...
  : [...aiMessages, { role: "user", content: AI_FOLLOW_UP_MESSAGES[aiRound] }];
  ...
  <div style={{ maxWidth:"480px", margin:"0 auto", padding:"0 20px" }}>
    ...
    <div style={{ background:"#fff", borderRadius:"16px", padding:"24px", marginTop:"16px", boxShadow:"0 2px 16px rgba(0,0,0,0.06)" }}>
      ...
      {!aiLoading && aiRound > 0 && aiRound < 3 && (
        <button onClick={fetchAiComment} style={{ width:"100%", marginTop:"20px", background:"#F5F1EC", color:"#4A9068", border:"1.5px solid #4A9068", borderRadius:"12px", padding:"14px", fontSize:"14px", fontWeight:"700", cursor:"pointer" }}>
          もう一度七海に聞く（あと{3 - aiRound}回）
        </button>
      )}
      {!aiLoading && aiRound >= 3 && (
